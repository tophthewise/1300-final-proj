<!DOCTYPE HTML>
<html>
	<head>
		<link rel="stylesheet" href="css/redsolo.css">
		<title> Red Solo Cupella </title>
        <link href='https://fonts.googleapis.com/css?family=Dosis' rel='stylesheet' type='text/css'>
	</head>
	<body>
        <nav class="nav centerme fatpad">
		  <ul class = " centerme ">
		  	  <li class="logo_pic"><img  src="images/redsolologo.png" alt=""></li>
              <li class="inline spaceme"><a  href="index.php">HOME</a></li>
              <li class="inline spaceme"><a  href="pages/members.php">MEMBERS</a></li>
              <li class="inline spaceme"><a  href="pages/performances.php">SHOWS</a></li>
              <li class="inline spaceme"><a  href="pages/media.php">MEDIA</a></li>
              <li class="inline spaceme"><a  href="pages/contact.php">CONTACT US</a></li>
		</ul>
	</nav>
	<br>
 <div class="coverphoto">
     <h3 class = "centerme"> A  Pop-Fusion <br> Acapella Group</h3>
		<img class =" pics" src="images/fullgroup.jpg" alt="A a full group photo from our photoshoot.">
     
        </div>
        <div class="citation">
<p class="foot"><a class="foot" href="http://chambermusicad.org/wp-content/uploads/2014/03/abstract-music-background.jpg">Background Image from chambermusicad.org Fonts courtsey of 1001 Fonts and Google Fonts 
</a> <!-- Background image from chambermusicad.org-->
</p>
        </div>
	</body>
</html>
